...
const designerReducer = (prevState = initialState, action) => {
  switch (action.type) {
    ...
    case NEW_ROW:
      return {
        ...prevState,
        art: prevState.art.length < 25 ? [...prevState.art, Array(prevState.art[0].length).fill(null)] : prevState.art,
      };

    case REMOVE_ROW:
      return {
        ...prevState,
        art: prevState.art.length > 3 ? prevState.art.slice(0, -1) : prevState.art,
      };

    case NEW_COL:
      return {
        ...prevState,
        art: prevState.art[0].length < 20 ? prevState.art.map((row) => [...row, null]) : prevState.art,
      };

    case REMOVE_COL:
      return {
        ...prevState,
        art: prevState.art[0].length > 3 ? prevState.art.map((row) => row.slice(0, -1)) : prevState.art,
      };

    case REMOVE_ART_ROW:
      return {
        ...prevState,
        art: prevState.art.map((row, idx) => (idx !== action.row ? row : row.map((col, colIdx) => null))),
      };

    case REMOVE_ART_COL:
      return {
        ...prevState,
        art: prevState.art.map((row, rowIdx) => row.map((col, colIdx) => (colIdx !== action.col ? col : null))),
      };
